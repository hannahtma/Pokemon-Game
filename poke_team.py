...
class PokeTeam:
    ...
    def random_team(cls, team_name: str, battle_mode: int, team_size=None, ai_mode=None, **kwargs):
        ...
        pokemon_team = ArrayR(team_size)
        pokemon_arranged = [Charmander, Bulbasaur, Squirtle, Gastly, Eevee]
        i = 0
        pokemon_total = 0
        for index in range(len(team_count)):
            if team_count[index] != 0:
                pokemon_total += team_count[index]
                number = 0
                while number < team_count[index]:
                    while i < pokemon_total:
                        pokemon_team.setitem(i, pokemon_arranged[index])
                        i += 1
                    number += 1
